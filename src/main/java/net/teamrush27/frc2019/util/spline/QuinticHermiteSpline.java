...
public class QuinticHermiteSpline extends Spline {
  ...
  public double getDCurvature(double t) {
    ...
    double num =
        (dx(t) * dddy(t) - dddx(t) * dy(t)) * dx2dy2
  ...
  private static void runOptimizationIteration(List<QuinticHermiteSpline> splines) {
    ...
    for (int i = 0; i < splines.size() - 1; ++i) {
      ...
      if (splines.get(i).getStartPose().isColinear(splines.get(i + 1).getStartPose())
          || splines.get(i).getEndPose().isColinear(splines.get(i + 1).getEndPose())) {
        continue;
      }
      ...
      magnitude +=
          controlPoints[i].ddx * controlPoints[i].ddx + controlPoints[i].ddy * controlPoints[i].ddy;
    }
    ...
    for (int i = 0;
        i < splines.size() - 1;
        ++i) { // first point is offset from the middle location by -stepSize
      if (splines.get(i).getStartPose().isColinear(splines.get(i + 1).getStartPose())
          || splines.get(i).getEndPose().isColinear(splines.get(i + 1).getEndPose())) {
        continue;
      }
    ...
    for (int i = 0;
        i < splines.size() - 1;
        ++i) { // last point is offset from the middle location by +stepSize
      if (splines.get(i).getStartPose().isColinear(splines.get(i + 1).getStartPose())
          || splines.get(i).getEndPose().isColinear(splines.get(i + 1).getEndPose())) {
        continue;
      }
      // move along the gradient by 2 times the step size amount (to return to original location and
      // move by 1
      // step)
    ...
    double stepSize =
        fitParabola(
            p1, p2, p3); // approximate step size to minimize sumDCurvature2 along the gradient
    ...
    for (int i = 0; i < splines.size() - 1; ++i) {
      ...
      // move by the step size calculated by the parabola fit (+1 to offset for the final
