 ...
 */
public class DifferentialDrive implements IDrivetrainModel {
  ...
  // Empirical testing of our drivebase showed that there was an unexplained loss in torque
  // ~proportional to angular
  // velocity, likely due to scrub of wheels.
  // NOTE: this may not be a purely linear term, and we have done limited testing, but this factor
  // helps our model to
  ...
  // "Effective" kinematic wheelbase radius.  Might be larger than theoretical to compensate for
  // skid steer.  Measure
  // by turning the robot in place several times and figuring out what the equivalent wheelbase
  // radius is.
  ...
  public ChassisState solveForwardKinematics(final WheelState wheel_motion) {
    ...
    chassis_motion.angular =
        wheel_radius_
            * (wheel_motion.right - wheel_motion.left)
            / (2.0 * effective_wheelbase_radius_);
  ...
  public WheelState solveInverseKinematics(final ChassisState chassis_motion) {
    ...
    wheel_motion.left =
        (chassis_motion.linear - effective_wheelbase_radius_ * chassis_motion.angular)
            / wheel_radius_;
    wheel_motion.right =
        (chassis_motion.linear + effective_wheelbase_radius_ * chassis_motion.angular)
            / wheel_radius_;
  ...
  public DriveDynamics solveForwardDynamics(
      final ChassisState chassis_velocity, final WheelState voltage) {
  ...
  public DriveDynamics solveForwardDynamics(
      final WheelState wheel_velocity, final WheelState voltage) {
    DriveDynamics dynamics = new DriveDynamics();
  ...
  public void solveForwardDynamics(DriveDynamics dynamics) {
    final boolean left_stationary =
        MathUtils.epsilonEquals(dynamics.wheel_velocity.left, 0.0)
            && Math.abs(dynamics.voltage.left) < left_transmission_.friction_voltage();
    final boolean right_stationary =
        MathUtils.epsilonEquals(dynamics.wheel_velocity.right, 0.0)
            && Math.abs(dynamics.voltage.right) < right_transmission_.friction_voltage();
    ...
    dynamics.wheel_torque.left =
        left_transmission_.getTorqueForVoltage(dynamics.wheel_velocity.left, dynamics.voltage.left);
    dynamics.wheel_torque.right =
        right_transmission_.getTorqueForVoltage(
            dynamics.wheel_velocity.right, dynamics.voltage.right);

    // Add forces and torques about the center of mass.
    dynamics.chassis_acceleration.linear =
        (dynamics.wheel_torque.right + dynamics.wheel_torque.left) / (wheel_radius_ * mass_);
    ...
    dynamics.dcurvature =
        (dynamics.chassis_acceleration.angular
                - dynamics.chassis_acceleration.linear * dynamics.curvature)
    ...
    dynamics.wheel_acceleration.left =
        dynamics.chassis_acceleration.linear
            - dynamics.chassis_acceleration.angular * effective_wheelbase_radius_;
  ...
  public DriveDynamics solveInverseDynamics(
      final ChassisState chassis_velocity, final ChassisState chassis_acceleration) {
    DriveDynamics dynamics = new DriveDynamics();
    dynamics.chassis_velocity = chassis_velocity;
    dynamics.curvature = dynamics.chassis_velocity.angular / dynamics.chassis_velocity.linear;
    if (Double.isNaN(dynamics.curvature)) {
  ...
  public DriveDynamics solveInverseDynamics(
      final WheelState wheel_velocity, final WheelState wheel_acceleration) {
    DriveDynamics dynamics = new DriveDynamics();
    dynamics.chassis_velocity = solveForwardKinematics(wheel_velocity);
    dynamics.curvature = dynamics.chassis_velocity.angular / dynamics.chassis_velocity.linear;
    if (Double.isNaN(dynamics.curvature)) {
  ...
  // Assumptions about dynamics: velocities and accelerations provided, curvature and dcurvature
  // computed.
  public void solveInverseDynamics(DriveDynamics dynamics) {
    // Determine the necessary torques on the left and right wheels to produce the desired wheel
    // accelerations.
    dynamics.wheel_torque.left =
        wheel_radius_
            / 2.0
            * (dynamics.chassis_acceleration.linear * mass_
    ...
    dynamics.voltage.left =
        left_transmission_.getVoltageForTorque(
            dynamics.wheel_velocity.left, dynamics.wheel_torque.left);
    dynamics.voltage.right =
        right_transmission_.getVoltageForTorque(
            dynamics.wheel_velocity.right, dynamics.wheel_torque.right);
  }

  public double getMaxAbsVelocity(
      double curvature, /*double dcurvature, */ double max_abs_voltage) {
    ...
    final double left_speed_at_max_voltage =
        left_transmission_.free_speed_at_voltage(max_abs_voltage);
    final double right_speed_at_max_voltage =
        right_transmission_.free_speed_at_voltage(max_abs_voltage);
    if (MathUtils.epsilonEquals(curvature, 0.0)) {
    ...
    final double right_speed_if_left_max =
        left_speed_at_max_voltage
            * (effective_wheelbase_radius_ * curvature + 1.0)
            / (1.0 - effective_wheelbase_radius_ * curvature);
    if (Math.abs(right_speed_if_left_max)
        <= right_speed_at_max_voltage + MathUtils.DEFAULT_MAX_ERROR) {
  ...
  // Curvature is redundant here in the case that chassis_velocity is not purely angular.  It is the
  // responsibility of
  // the caller to ensure that curvature = angular vel / linear vel in these cases.
  public MinMax getMinMaxAcceleration(
      final ChassisState chassis_velocity,
      double curvature, /*double dcurvature,*/
      double max_abs_voltage) {
    ...
    final double linear_term =
        Double.isInfinite(curvature) ? 0.0 : mass_ * effective_wheelbase_radius_;
    ...
    for (boolean left : Arrays.asList(false, true)) {
      for (double sign : Arrays.asList(1.0, -1.0)) {
        final DCMotorTransmission fixed_transmission =
            left ? left_transmission_ : right_transmission_;
        final DCMotorTransmission variable_transmission =
            left ? right_transmission_ : left_transmission_;
        final double fixed_torque =
            fixed_transmission.getTorqueForVoltage(
                wheel_velocities.get(left), sign * max_abs_voltage);
        double variable_torque = 0.0;
        // NOTE: variable_torque is wrong.  Units don't work out correctly.  We made a math error
        // somewhere...
        // Leaving this "as is" for code release so as not to be disingenuous, but this whole
        // function needs
        // revisiting in the future...
        if (left) {
          variable_torque =
              ((
                          /*-moi_ * chassis_velocity.linear * chassis_velocity.linear * dcurvature*/ -drag_torque)
                          * mass_
                          * wheel_radius_
        ...
        final double variable_voltage =
            variable_transmission.getVoltageForTorque(wheel_velocities.get(!left), variable_torque);
        if (Math.abs(variable_voltage) <= max_abs_voltage + MathUtils.DEFAULT_MAX_ERROR) {
          double accel = 0.0;
          if (Double.isInfinite(curvature)) {
            accel =
                (left ? -1.0 : 1.0)
                        ...
                        * effective_wheelbase_radius_
                        / (moi_ * wheel_radius_)
                    - drag_torque
                        / moi_ /*- chassis_velocity.linear * chassis_velocity.linear * dcurvature*/;
          } else {
