...
public class TimingUtil {
  public static <S extends State<S>> Trajectory<TimedState<S>> timeParameterizeTrajectory(
    ...
    for (int i = 0; i < num_states; ++i) {
      states.add(
          distance_view.sample(Math.min(i * step_size, distance_view.last_interpolant())).state());
    }
  ...
  public static <S extends State<S>> Trajectory<TimedState<S>> timeParameterizeTrajectory(
    ...
    // Forward pass. We look at pairs of consecutive states, where the start state has already been
    // velocity
    // parameterized (though we may adjust the velocity downwards during the backwards pass). We
    // wish to find an
    // acceleration that is admissible at both the start and end state, as well as an admissible end
    // velocity. If
    // there is no admissible end velocity or acceleration, we set the end velocity to the state's
    // maximum allowed
    // velocity and will repair the acceleration during the backward pass (by slowing down the
    // predecessor).
    ...
    for (int i = 0; i < states.size(); ++i) {
      ...
      // We may need to iterate to find the maximum end velocity and common acceleration, since
      // acceleration
      ...
      while (true) {
        ...
        // At this point, the state is full constructed, but no constraints have been applied aside
        // from
        // predecessor
        // state max accel.
        ...
        for (final TimingConstraint<S> constraint : constraints) {
          constraint_state.max_velocity =
              Math.min(
                  constraint_state.max_velocity, constraint.getMaxVelocity(constraint_state.state));
        }
        ...
        for (final TimingConstraint<S> constraint : constraints) {
          final TimingConstraint.MinMaxAcceleration min_max_accel =
              constraint.getMinMaxAcceleration(
                  constraint_state.state, (reverse ? -1.0 : 1.0) * constraint_state.max_velocity);
        ...
        // If the max acceleration for this constraint state is more conservative than what we had
        // applied, we
        // need to reduce the max accel at the predecessor state and try again.
        // TODO: Simply using the new max acceleration is guaranteed to be valid, but may be too
        // conservative.
        // Doing a search would be better.
        final double actual_acceleration =
            (constraint_state.max_velocity * constraint_state.max_velocity
                    - predecessor.max_velocity * predecessor.max_velocity)
                / (2.0 * ds);
        if (constraint_state.max_acceleration < actual_acceleration - kEpsilon) {
          ...
          // If actual acceleration is less than predecessor min accel, we will repair during the
          // backward
          // pass.
          break;
        }
    ...
    for (int i = states.size() - 1; i >= 0; --i) {
      ...
      while (true) {
        ...
        // If the min acceleration for this constraint state is more conservative than what we have
        // applied, we
        // need to reduce the min accel and try again.
        // TODO: Simply using the new min acceleration is guaranteed to be valid, but may be too
        // conservative.
        // Doing a search would be better.
        final double actual_acceleration =
            (constraint_state.max_velocity * constraint_state.max_velocity
    ...
    for (int i = 0; i < states.size(); ++i) {
      ...
      final double accel =
          (constrained_state.max_velocity * constrained_state.max_velocity - v * v) / (2.0 * ds);
      double dt = 0.0;
      if (i > 0) {
      ...
      timed_states.add(
          new TimedState<>(constrained_state.state, t, reverse ? -v : v, reverse ? -accel : accel));
    }
