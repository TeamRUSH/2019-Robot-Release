...
import net.teamrush27.frc2019.util.math.IPose2d;
...
import net.teamrush27.frc2019.util.motion.IPathFollower;
import net.teamrush27.frc2019.util.spline.QuinticHermiteSpline;
import net.teamrush27.frc2019.util.spline.Spline;
import net.teamrush27.frc2019.util.spline.SplineGenerator;
...
public class TrajectoryUtil {
  ...
  public static <S extends IPose2d<S>> Trajectory<TimedState<S>> mirrorTimed(
  ...
  public static <S extends IPose2d<S>> Trajectory<S> transform(
      final Trajectory<S> trajectory, Pose2d transform) {
    List<S> waypoints = new ArrayList<>(trajectory.length());
    for (int i = 0; i < trajectory.length(); ++i) {
  ...
  public static <S extends State<S>> Trajectory<S> resample(
    ...
    if (interval <= MathUtils.DEFAULT_MAX_ERROR) {
  ...
  public static Trajectory<Pose2dWithCurvature> trajectoryFromPathFollower(
    ...
    while (!path_follower.isDone()) {
      ...
      final double dcurvature =
          (steering_command.curvature() - current_state.getCurvature()) / steering_command.norm();
      final boolean curvature_valid =
          !Double.isNaN(dcurvature)
              && !Double.isInfinite(dcurvature)
              && !Double.isNaN(current_state.getCurvature())
              && !Double.isInfinite(current_state.getCurvature());
      if (dcurvature > dcurvature_limit && curvature_valid) {
        steering_command =
            new Twist2d(
                steering_command.deltaX,
                steering_command.deltaY,
      ...
      final Twist2d average_steering_command =
          !curvature_valid
              ? steering_command
              : new Twist2d(
                  steering_command.deltaX,
