...
import net.teamrush27.frc2019.util.trajectory.TrajectoryPoint;
import net.teamrush27.frc2019.util.trajectory.TrajectorySamplePoint;
import net.teamrush27.frc2019.util.trajectory.TrajectoryView;
  ...
  public TrajectorySamplePoint<TimedState<S>> sample(double t) {
    ...
    for (int i = 1; i < trajectory_.length(); ++i) {
      ...
      if (s.state().t() >= t) {
        ...
        if (MathUtils.epsilonEquals(s.state().t(), prev_s.state().t())) {
