...
import net.teamrush27.frc2019.util.physics.DCMotorTransmission;
import net.teamrush27.frc2019.util.physics.DifferentialDrive;
import net.teamrush27.frc2019.util.physics.IDrivetrainModel.ChassisState;
import net.teamrush27.frc2019.util.physics.IDrivetrainModel.DriveDynamics;
import net.teamrush27.frc2019.util.physics.IDrivetrainModel.WheelState;
import net.teamrush27.frc2019.util.trajectory.DistanceView;
import net.teamrush27.frc2019.util.trajectory.Trajectory;
import net.teamrush27.frc2019.util.trajectory.TrajectoryIterator;
import net.teamrush27.frc2019.util.trajectory.TrajectorySamplePoint;
import net.teamrush27.frc2019.util.trajectory.TrajectoryUtil;
import net.teamrush27.frc2019.util.trajectory.timing.DifferentialDriveDynamicsConstraint;
...
public class DriveMotionPlanner implements CSVWritable {
  ...
  public DriveMotionPlanner() {
    final DCMotorTransmission transmission =
        new DCMotorTransmission(
            ...
            Units.inches_to_meters(ChezyConstants.kDriveWheelRadiusInches)
                * Units.inches_to_meters(ChezyConstants.kDriveWheelRadiusInches)
                * ChezyConstants.kRobotLinearInertia
                / (2.0 * Robot.ROBOT_CONFIGURATION.getDriveKa()),
        ...
        new DifferentialDrive(
            ...
            Units.inches_to_meters(
                ChezyConstants.kDriveWheelTrackWidthInches
                    / 2.0
                    * Robot.ROBOT_CONFIGURATION.getScrubFactor()),
            transmission,
            transmission);
  }
  ...
  public void setTrajectory(final TrajectoryIterator<TimedState<Pose2dWithCurvature>> trajectory) {
    ...
    for (int i = 0; i < trajectory.trajectory().length(); ++i) {
      if (trajectory.trajectory().getState(i).velocity() > MathUtils.DEFAULT_MAX_ERROR) {
        mIsReversed = false;
        break;
      } else if (trajectory.trajectory().getState(i).velocity() < -MathUtils.DEFAULT_MAX_ERROR) {
        mIsReversed = true;
        break;
      }
    }
  ...
  public Trajectory<TimedState<Pose2dWithCurvature>> generateTrajectory(
    ...
    return generateTrajectory(
        reversed, waypoints, constraints, 0.0, 0.0, max_vel, max_accel, max_voltage);
  }
  ...
  public Trajectory<TimedState<Pose2dWithCurvature>> generateTrajectory(
    ...
    final TimingConstraint<Pose2dWithCurvature> drive_constraints =
  ...
  public String header(String base) {
    return base
        ...
        + base
        + "_r_ff_v,"
        + mSetpoint.header(base + "_setpoint");
  }
  ...
  public static class Output {
    ...
    public String toString() {
      ...
      return String.format(
          "(%s, %s) (%s, %s) (%s, %s)",
          fmt.format(left_velocity),
          fmt.format(right_velocity),
          fmt.format(left_accel),
          fmt.format(right_accel),
          fmt.format(left_feedforward_voltage),
          fmt.format(right_feedforward_voltage));
    }
  }
  ...
  protected Output updatePID(DriveDynamics dynamics, Pose2d current_state) {
    ...
    final WheelState wheel_velocities = mModel.solveInverseKinematics(adjusted_velocity);
                  ...
                  lookahead_state
                      ...
                      .transformBy(
                          Pose2d.fromTranslation(
                              new Translation2d(
                                  (mIsReversed ? -1.0 : 1.0)
                                      * (ChezyConstants.kPathMinLookaheadDistance
  ...
  protected Output updateNonlinearFeedback(
      DifferentialDrive.DriveDynamics dynamics, Pose2d current_state) {
    ...
    final double k =
        2.0
            * kZeta
            * Math.sqrt(
                kBeta * dynamics.chassis_velocity.linear * dynamics.chassis_velocity.linear
    ...
    final double sin_x_over_x =
        MathUtils.epsilonEquals(angle_error_rads, 0.0, 1E-2)
            ? 1.0
            : mError.getRotation().sin() / angle_error_rads;
    final DifferentialDrive.ChassisState adjusted_velocity =
    ...
    dynamics.chassis_acceleration.linear =
        mDt == 0 ? 0.0 : (dynamics.chassis_velocity.linear - prev_velocity_.linear) / mDt;
    dynamics.chassis_acceleration.angular =
        mDt == 0 ? 0.0 : (dynamics.chassis_velocity.angular - prev_velocity_.angular) / mDt;

    prev_velocity_ = dynamics.chassis_velocity;

    DifferentialDrive.WheelState feedforward_voltages =
        mModel.solveInverseDynamics(dynamics.chassis_velocity, dynamics.chassis_acceleration)
            .voltage;
  ...
  public Output update(double timestamp, Pose2d current_state) {
    ...
    TrajectorySamplePoint<TimedState<Pose2dWithCurvature>> sample_point =
        mCurrentTrajectory.advance(mDt);
    ...
    if (!mCurrentTrajectory.isDone()) {
      ...
      final double dcurvature_ds_m =
          Units.meters_to_inches(Units.meters_to_inches(mSetpoint.state().getDCurvatureDs()));
      final double acceleration_m = Units.inches_to_meters(mSetpoint.acceleration());
      final DriveDynamics dynamics =
          mModel.solveInverseDynamics(
