...
import net.teamrush27.frc2019.util.math.ITranslation2d;
...
public class PurePursuitController<S extends ITranslation2d<S>> implements IPathFollower {
  ...
  public PurePursuitController(
      final DistanceView<S> path, double sampling_dist, double lookahead, double goal_tolerance) {
  ...
  public Twist2d steer(final Pose2d current_pose) {
    done_ =
        done_
            || (iterator_.isDone()
                && current_pose.getTranslation().distance(iterator_.getState().getTranslation())
                    <= goal_tolerance_);
    if (isDone()) {
    ...
    // Find the first point > lookahead distance away from current_pose, or the last point
    // otherwise.
    for (double progress = 0.0;
        progress <= remaining_progress;
        progress = Math.min(remaining_progress, progress + sampling_dist_)) {
      double dist =
          current_pose
              .getTranslation()
              .distance(iterator_.preview(progress).state().getTranslation());
      if (dist > lookahead_) {
        if (goal_progress == 0.0 && !iterator_.isDone()) {
          // Make sure we don't get stuck due to numerical issues when sampling dist is large
          // relative to
          // lookahead.
    ...
    if (arc.length < MathUtils.DEFAULT_MAX_ERROR) {
      return new Twist2d(0.0, 0.0, 0.0);
    } else {
  ...
  protected static <S extends ITranslation2d<S>> double getDirection(Pose2d pose, S point) {
    ...
    return Math.signum(cross); // if robot < pose turn left
  }
  ...
  public static class Arc<S extends ITranslation2d<S>> {
    ...
    protected Translation2d findCenter(Pose2d pose, S point) {
      final Translation2d poseToPointHalfway =
          pose.getTranslation().interpolate(point.getTranslation(), 0.5);
      final Rotation2d normal =
          pose.getTranslation().inverse().translateBy(poseToPointHalfway).direction().normal();
    ...
    protected double findLength(Pose2d pose, S point, Translation2d center, double radius) {
      if (radius < Double.MAX_VALUE) {
        ...
        // point is behind,
        // check the sign of the cross-product between the normal vector and the vector from pose to
        // point.
        final boolean behind =
            Math.signum(
                    Translation2d.cross(
                        pose.getRotation().normal().toTranslation(),
                        new Translation2d(pose.getTranslation(), point.getTranslation())))
                > 0.0;
