...
public class CheesyDriveHelper {
  ...
  private static final double kQuickStopScalar = 2.5;

  private double mOldWheel = 0.0;
  private double mQuickStopAccumlator = 0.0;
  private double mNegInertiaAccumlator = 0.0;

  public DriveCommand cheesyDrive(
    ...
    if (isQuickTurn) {
      if (Math.abs(linearPower) < kQuickStopDeadband) {
        ...
        mQuickStopAccumlator =
            (1 - alpha) * mQuickStopAccumlator
                + alpha * MathUtils.limit(wheel, 1.0) * kQuickStopScalar;
      }
      overPower = 1.0;
      angularPower = wheel;
    } else {
    ...
    return new DriveCommand(leftPwm, rightPwm);
  }
